...
class CoursesApiTests(ModuleStoreTestCase):
    ...
    def test_courses_data_time_series_metrics(self):
        ...
        enrolled_time = datetime.now() - timedelta(days=25)
        ...
        for j, user in enumerate(users):
            ...
            submit_time = datetime.now() - timedelta(days=(USER_COUNT - j))
            with freeze_time(submit_time):
                module = self.get_module_for_user(user, course, item)
                grade_dict['user_id'] = user.id
                module.system.publish(module, 'grade', grade_dict)

                # For the first two users, submit an score for the second module
                if j >= USER_COUNT - 2:
        ...
        for j, user in enumerate(users[:5]):
            submit_time = datetime.now() - timedelta(days=(5 - j))
            with freeze_time(submit_time):
                grade_dict['user_id'] = user.id
                second_module = self.get_module_for_user(user, course, item2)
                second_module.system.publish(second_module, 'grade', grade_dict)
        ...
        end_date = datetime.now() - timedelta(days=(USER_COUNT - 4))
        start_date = datetime.now() - timedelta(days=USER_COUNT)
        ...
        self.assertEqual(total_not_started, 105)  # 105 aggregate total in the first five days (25, 24, 23, 22, 21)
        ...
        # Change the time interval to five months, so we're back to five entries per category
        ...
        self.assertEqual(response.data['users_enrolled'][2][1], 25)
        total_enrolled = sum([enrolled[1] for enrolled in response.data['users_enrolled']])
